...
class WhisperIO(object):
    ...
    def write_media(self, audio_arrays, sampling_rates):
        """将音频数据写入文件或其他存储"""
        raise NotImplementedError("WhisperIO does not support write_media operation.")
    ...
    def _conversation_to_message(
        ...
        system_prompt: str = "<|startoftranscript|>"
        ):
        """
        将格式1的数据转换为格式2。
    ...
    def _parse_multimodal_tokens(self, text):
        ...
        cleaned_text = self.system_prompt
